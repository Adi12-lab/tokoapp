        ...
        <div class="container-md">
            ...
            <div class="row">
                <div class="col-md-5">
                    <div id="sync1" class="owl-carousel owl-theme">
                        @foreach ($product->productGallery->where('jenis', 'carousel') as $crl)
                            <div class="item">
                                <img src="{{ asset('storage/' . $crl->gambar) }}" class="img-thumbnail img-fluid"
                                    alt="{{ $crl->name }}">
                            </div>
                        @endforeach
                    </div>
                ...
                <div class="col-md-5 mt-3">
                    ...
                    <span class="product-price fs-2">{{ rupiah($product->size[0]->price) }}</span>
                    <span
                        class="product-price-old text-decoration-line-through fs-5">{{ rupiah($product->size[0]->old_price) }}</span>
                        ...
                        <div class="row mt-2">
                            ...
                            <div>
                                @foreach ($product->variant as $var)
                                    @if ($loop->first)
                                        <a class="btn option variant active"
                                            data-variant="{{ $var->name }}">{{ $var->name }}</a>
                                        @continue
                                    @endif
                        ...
                        <div class="row size">
                            ...
                            <div>
                                ...
                                @foreach ($product->size as $siz)
                                    @if ($loop->first)
                                        <a class="btn option size active" data-size="{{ $siz->name }}"
                                            data-price-size="{{ $siz->price }}"
                                            data-weight-size="{{ $siz->weight }}">{{ $siz->name }} :
                                            {{ rupiah($siz->price ?? 0) }}</a>
                                        @continue
                                    @endif
                                    <a class="btn option size" data-size="{{ $siz->name }}"
                                        data-price-size="{{ $siz->price }}"
                                        data-weight-size="{{ $siz->weight }}">{{ $siz->name }} :
                                        {{ rupiah($siz->price) }} </a>
                                @endforeach
                    ...
                    <div class="row mt-3">
                        <div class="col d-flex align-items-center">
                            ...
                            <button class="btn btn-primary px-3 py-2 ms-3 addCart body"><i class="bi bi-cart"></i>
                                Keranjang</button>
